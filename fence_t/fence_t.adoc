...
One way of enforcing _fence.t_ semantics is to have instructions executing in constant time.
...
One way to address this would be to force _fence.t_ to execute in constant time. Alternatively, the privileged software could contain a delay loop that pads execution time to a constant value. However, in practice this software padding may be difficult to do accurately.
